...
load_file = True
filename = 'strategies_1line_diffstart_ben15.npz'

# Lori is strategy 1, Ben is strategy 2
cost_per_bet = [0.3, 0.15]
...
ax.hist(avg_money[0, :], bins=20, rwidth=1, normed=True, label='Lori', alpha = 0.7)
ax.set_title('Earnings Distribution')
ax.set_ylabel('Frequency')
ax.set_xlabel('Earnings')
ax.hist(avg_money[1, :], bins=20, rwidth=1, normed=True, label='Ben', alpha = 0.7)
ax.legend()

fig, ax = plt.subplots(1, 1)
ax.bar([0, 1], utility)
ax.set_title(r'Utility vs Strategy')
ax.set_xlabel('Strategy')
ax.set_ylabel(r'Utility')
ax.set_xticks([0, 1])
ax.set_xticklabels(['Lori', 'Ben'])
